...
static void rane_x64_emit_mov_rax_imm64(rane_x64_emitter_t* e, uint64_t imm) {
  uint8_t rex = 0x48;
  uint8_t op = 0xB8; // MOV RAX, imm64
  rane_x64_emit_bytes(e, &rex, 1);
  rane_x64_emit_bytes(e, &op, 1);
  rane_x64_emit_bytes(e, (uint8_t*)&imm, 8);
}

static void rane_x64_emit_call_rax(rane_x64_emitter_t* e) {
  // FF /2 r/m64, modrm 11 010 000 => 0xD0
  uint8_t op = 0xFF;
  uint8_t modrm = 0xD0;
  rane_x64_emit_bytes(e, &op, 1);
  rane_x64_emit_bytes(e, &modrm, 1);
...
static void rane_x64_emit_prologue(rane_x64_emitter_t* e) {
  // push rbp; mov rbp, rsp
  rane_x64_emit_push_reg(e, 5);
  uint8_t rex = 0x48;
  uint8_t op = 0x89;
  uint8_t modrm = 0xE5; // mov rbp, rsp
  rane_x64_emit_bytes(e, &rex, 1);
  rane_x64_emit_bytes(e, &op, 1);
  rane_x64_emit_bytes(e, &modrm, 1);
}
...
static void rane_x64_emit_epilogue(rane_x64_emitter_t* e) {
  ...
  uint8_t rex = 0x48;
  uint8_t op = 0x89;
  uint8_t modrm = 0xEC; // mov rsp, rbp
  rane_x64_emit_bytes(e, &rex, 1);
  rane_x64_emit_bytes(e, &op, 1);
  rane_x64_emit_bytes(e, &modrm, 1);
