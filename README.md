...
The repository currently contains a **bootstrap compiler** targeting **Windows x64**, producing a minimal **PE executable**. The focus of this phase is correctness, determinism, and a clean end‑to‑end pipeline.

RANE is under active development. Several advanced features described in older documents (e.g., richer type annotations, capsules, full memory‑band semantics) are **planned** but not yet implemented.
...
This stage is about establishing a deterministic, analyzable foundation for future expansion.
...
Built into the parser and lowered directly into TIR:
...
These provide explicit, low‑level memory access in the bootstrap compiler.
...
- A minimal but real language surface  
- Deterministic lowering and fixups  
- A typed IR stage  
- A working native x64 backend  
- A prototype memory‑band loader concept  

Advanced features described in older documents are **planned**, not yet implemented.
